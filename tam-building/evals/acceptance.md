...
- The ICP was read from the workspace context repository before the operator was
  ...
  and the answer was written to `infra/context/icp.md`.
...
- The tier rubric was proposed in the ICP's own language, corrected by the
  operator, and written to `infra/context/tiering-rubric.md`. It is not in the
...
- Phase three reports rows landed against `limit`, the tier distribution, the
  disqualified share read as a verdict on the filter, the evaluator pass rate
  with a failing sample, actual credits against estimate, and one recommended
  next step.
...
- `tam-tier-analyst` carries the read-only `context` capability and `webSearch`,
  ...
  evaluator rubric with a threshold.
...
- `tier`, `tier_rationale` and `tiered_at` are written on that same node, so a
  row cannot be stamped without carrying its judgment.
