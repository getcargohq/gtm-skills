# Audit CRM account and contact duplicates

Use this reference before editing CDK. The audit is read-only: inspect live CRM company and contact
properties without paid calls, review requests, or CRM writes.
...
Audit `crm_contacts` independently from accounts. Candidate keys are normalized LinkedIn person ID,
LinkedIn person URL, exact email, and phone. Person name, job title, company association, and fuzzy
similarity are evidence for a reviewer only. Report total contacts, coverage per key, unique groups,
affected contacts, high-confidence groups, phone-only groups, generic/shared-email groups, LinkedIn
conflicts, and stale or invalid records.

Normalize email by trimming and lowercasing. Normalize LinkedIn URLs to the `/in/<handle>` handle and
audit all four runtime URL forms with and without `www` and a trailing slash. Phone matching must
cover explicit international numbers, `00` prefixes, NANP numbers, and the approved French and UK
local forms. Phone-only matches stay low confidence even after normalization.

The contact automatic classes are exact LinkedIn person ID, exact LinkedIn URL without a person-ID
conflict, exact non-generic email without LinkedIn conflict, and conflict-free transitive chains of
those classes. Apply LinkedIn conflict and generic/shared-email guards to the whole automatic
decision, including clusters that also share a person ID. Rank one survivor by associated deals,
activity, oldest creation time, populated properties, then CRM record ID.

The contact audit is complete when every group has distinct CRM IDs, one deterministic survivor,
one match class, the global conflict flags, and the exact IDs proposed for merge. The account and
contact JSON, Markdown, and chat summaries must each reconcile to their source totals.
