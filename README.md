...
`crm-enrichment`, `call-capture`, `agentic-engagement` and `sending-infrastructure` are the same jobs as a standing pipeline: each folder holds worked CDK
...
| [`sending-infrastructure`](sending-infrastructure/SKILL.md) | Cold-email capacity sized to a target volume: lookalike domains with DMARC and a redirect, a grid of mailboxes on real senders, warm-up started on every one. |
