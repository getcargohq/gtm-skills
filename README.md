...
**Six include reusable CDK resources.** `tam-building`, `account-scoring`,
`crm-enrichment`, `call-capture`, `agentic-engagement` and `contact-sourcing` build standing pipelines or on-demand tools: each folder holds worked CDK
...
already declares. More are on their way (`signal-based-tam`, `ai-sdr`,
...
| [`contact-sourcing`](contact-sourcing/SKILL.md) | On-demand stakeholder sourcing: qualify responsibilities and rank contact fit, with optional email/phone enrichment after selection. |
