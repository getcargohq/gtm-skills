...
`crm-enrichment`, `call-capture`, `agentic-engagement` and `reply-inbox` are the same jobs as a standing pipeline: each folder holds worked CDK
...
| [`reply-inbox`](reply-inbox/SKILL.md)                      | A hosted app the team lives in: mailbox replies in one queue, a reply that stays on the thread, LinkedIn send when a connection exists. |
