...
the surface on top of everything the other cookbooks computed.

## What it does

- Lists the rep's book — tier-A accounts sorted by score — from the base
  `accounts` model.
- Shows what the other cookbooks produced per account: `cargo_score`
...
routing-engine ─ territory ─────┼─▶ base-gtm: accounts (model)
...
2. **Columns** — `cargo_score`, `territory`, `outreach_draft`, `next_action`
   come from the other cookbooks; deploy those first (or trim the query).
