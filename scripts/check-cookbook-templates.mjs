...
const variants = [
  ["account-enrichment", "hubspot"],
  ["account-enrichment", "salesforce"],
  ["account-enrichment", "attio"],
...
for (const [cookbook, crm] of variants) {
  ...
  try {
    ...
    // Cargo's loader accepts the historical, flat per-CRM module graph. The
    // repository groups those same files by resource type, so assemble one
    // selected CRM variant into that graph only in this disposable check dir.
    ...
    execFileSync(
      join(root, "node_modules", ".bin", "cargo-cdk"),
      ["check", "--dir", target],
      { stdio: "pipe" },
    );
