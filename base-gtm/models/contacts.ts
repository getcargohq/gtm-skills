...
// identity key.
//
// NATIVE, like accounts: no connector, no env var, so base-gtm deploys on a
// fresh workspace. `defineContact` gives the standard contact schema (name,
// email, title, department, account_id, ...).
//
// Fill it with contact-sourcing, or source it from your CRM: install `crm-sync`
...
export const contacts = defineModel("contacts", {
  ...
  // owner_id, mailing_*, description. These add what the cookbooks write.
  additionalColumns: [
    {
      kind: "custom",
