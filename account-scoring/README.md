...
- Keeps the scoring criteria where the ICP already lives: the context repo
  (`gtm-knowledge-graph/context/icp.md`) — versioned in git, reviewable, and
...
workflow), and 2 segments. Requires `gtm-knowledge-graph` (the ICP the agent
scores against).
...
1. **The ICP itself** — `gtm-knowledge-graph/context/icp.md`: the criteria ARE
