...
// The account universe: the model every cookbook builds on (the signals feed
...
// source, so base-gtm deploys with no connector and no env var. `kind: "native"`
...
//
// Contract for downstream cookbooks: a string `domain` column as the identity
// key, and ideally a LinkedIn company URL.
//
// Fill it by installing a sourcing cookbook (tam-building, contact-sourcing), or
// source it from your CRM instead: install `crm-sync` and swap this model for a
...
export const accounts = defineModel("accounts", {
  ...
  //
  // These are the columns the cookbooks write, declared once here so the shared
  ...
  additionalColumns: [
    {
      ...
      label: "Score",
      description:
        "Fit + intent score, written by the account-scoring cookbook.",
    },
    {
      kind: "custom",
      slug: "cargo_tier",
