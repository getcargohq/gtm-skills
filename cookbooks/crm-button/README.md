...
they are staring at changes while they watch. If you adapt this cookbook, keep
the write-back step.

## What's inside

Adds one tool on top of the base, and uses the CRM connector from `crm-sync`.

| File                     | Resource     | Role                                                  |
| ------------------------ | ------------ | ----------------------------------------------------- |
| `tools/enrich-record.ts` | `defineTool` | reads the record, enriches it, writes the result back |

This is the first cookbook in the repo to use `publicForm`, which is what makes a
...
Any cookbook can become a button: `research-agent` (brief this account),
