...
customer status. `enrich_contacts` fills approved person attributes for non-customer contacts on
the six-month window. `monitor_champions` watches contacts whose primary company is a customer on
a 30-day window, compares their live LinkedIn company against the CRM company, and on a job change
updates the same contact, preserves the former company relationship, and alerts the former
...
mirrored onto contacts by HubSpot's native lifecycle sync; Salesforce:
`Contact.AccountId → Account.<customer status field>`), always through the contact's primary
company relationship, never an arbitrary associated company. The operator confirms that mapping
...
2. **Disabled plays and tool.** After that approval, adapt, type, check, plan, and deploy the reusable
   enrichment tool and the play or plays for the audited path, every play still disabled. The tool
...
| `customer_status_mapping` | derived, confirmed | Detect the live property and value that mean "customer" through the contact's primary company relationship, then present it for confirmation         | It decides which contacts `enrich_contacts` and `monitor_champions` each own      |
...
| `champion_alert_channel`  | asked              | Name the Slack channel that receives champion job-change alerts (people path only)                                                                   | The alert is the champion play's deliverable; a wrong channel loses the warm lead |
...
| `champion_extensions`       | The operator wants the job-change note on the CRM timeline, automatic company creation, or a downstream champion sequence | After `cargo-ai cdk types` confirms the live CRM's note-creation and record-creation actions, extend the champion play's job-change branch; hand the alert off to a play built on `track-job-changes` or `monitor-buying-signals` for sequencing                                                                                                                                                                                                                                            | Each extension adds CRM writes that must be typed and re-planned; record creation can mint duplicates if the find step is weakened first |
...
- **The customer-status split is the two contact filters.** (`infra/index.ts`) `enrich_contacts` owns non-customer contacts on the six-month window; `monitor_champions` owns customer-company contacts on the 30-day window, per the confirmed customer-status mapping. Widening either filter enrolls the same contact in both plays and bills it twice per cycle; narrowing both drops contacts into a gap nobody refreshes.
- **One person is one contact.** (`infra/index.ts` `monitorCrmChampion`) A job change updates the resolved existing contact — found by LinkedIn person identity, else the triggering row — moves the primary company, and preserves the former relationship. The play never creates, merges, or deletes a contact, and a new employer or work email is never a new identity. Breaking this splits one person across duplicate records and the champion history with them.
- **Job-change proof is LinkedIn company ID first, domain second.** (`infra/index.ts` `monitorCrmChampion`) A work email is never the person identity, and an email-domain mismatch alone is never the proof of a move. Loosening this fires champion alerts on rebrands and mail migrations, and the owner learns to ignore them.
...
- the CDK plan contains one CRM model per audited object (`crm_accounts`, `crm_contacts`) and no
  native `accounts` or `contacts` unification
- the agent installed and read the `cargo-cdk` skill before auditing or adapting the template
- the CDK plan contains the reusable tools (`account_enrichment`, `contact_enrichment`) and the
  disabled plays for the audited path; every play contains one Tool node targeting its tool before
  any CRM action, and only plays contain CRM actions
- `node --import tsx evals/contract.mjs` passes against the adapted compiled graphs; each tool
  ...
  unresolved rows before the person enrichment, and no play contains a provider action
- generated consumer types confirm the selected provider fields, CRM destinations, write and read
  actions, the Slack alert payload, and fill-blank semantics — including the provider result paths
  this repository marks `PLACEHOLDER`
...
- on a verified job change, the champion play updated the resolved existing contact, moved the
  primary company, preserved the former relationship, refreshed the title, and posted the
...
for `integration.actions.enrichProfile.credits.costs`, plus
`cargo-ai connection integration get FullEnrich` for
`integration.actions.reverseEmailLookup.credits.costs`. Record the CLI version, lookup time,
...
enrichment. CRM reads and the Slack alert are not credit-billed.
