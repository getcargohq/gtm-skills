...
version: "0.6.4"
compatibility: "Requires the cargo-cdk skill, a Cargo CDK project, and @cargo-ai/cdk 1.0.58 or later — 1.0.58 renamed `defineConnector`'s `adopt` to `default`, which both connectors here use to bind the workspace's existing CRM and LinkedIn connections. The repository example does not deploy or access a CRM until an agent adapts it in the consumer project."
